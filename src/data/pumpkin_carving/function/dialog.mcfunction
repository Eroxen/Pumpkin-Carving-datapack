from eroxified2:core import call_on_player_joined, run_at_pack_tick
from beet import Dialog

function ~/enable_triggers:
  call_on_player_joined()
  scoreboard players enable @s pumpkin_carving.dialog

function ~/tick:
  run_at_pack_tick()
  execute as @a[scores={pumpkin_carving.dialog=1..}] at @s run function ~/../triggered

function ~/triggered:
  execute run function ~/tree:
    execute if score @s pumpkin_carving.dialog matches 101 run return run function pumpkin_carving:statistics/show_dialog

    # any other value just shows the main menu
    raw dialog show @s pumpkin_carving:main_menu
  scoreboard players reset @s pumpkin_carving.dialog
  scoreboard players enable @s pumpkin_carving.dialog


ctx.data["pumpkin_carving"].dialogs["report_an_issue"] = Dialog({
  ...
  "actions": [
    ...
    },
    {
      ...
      "action": {
        "type": "minecraft:open_url",
        "url": "https://github.com/Eroxen/Pumpkin-Carving-datapack/issues"
      }
    }
  ]
})